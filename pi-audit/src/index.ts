...
async function updateAllCommand(rawArgs: string[]) {
  ...
  console.log(
    `${entries.length} managed package(s), ${updatable.length} update(s) available, ${current} current, ${skipped} skipped.`,
  )
...
function installSnapshotDependencies(snapshotPath: string) {
  ...
  run(
    executable,
    [...commandParts.slice(1), 'install', '--omit=dev', '--ignore-scripts'],
    snapshotPath,
  )
}
