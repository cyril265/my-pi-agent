...
export function extractSummaryLines(result: AgentToolResult<any>): {
  progress: string[]
  todo?: string
} {
  const firstText = result.content.find(part => part.type === 'text')
  if (!firstText || firstText.type !== 'text') return { progress: [] }
  const lines = firstText.text
    .split(/\r?\n/)
    .map(line => line.trim())
    .filter(Boolean)
  const progress = lines.filter(line =>
    /^(Plan|Done|Now|Next|Running|Queued|Failed|Status): /.test(line),
  )
...
export function withTodoTrackingNote(
