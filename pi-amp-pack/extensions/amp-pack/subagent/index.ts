...
const MAX_CHAIN_PREVIOUS_CHARS = 12_000
...
function resolveAgentModel(
  ...
  const preferredModel = preferredProvider
    ? modelRegistry.find(preferredProvider, configuredModel)
    : undefined
  if (preferredModel) {
...
function resolveRequestedModel(
  ...
  const preferredModel =
    preferredProvider && modelRegistry
      ? modelRegistry.find(preferredProvider, configuredModel)
      : undefined
...
function formatResultMeta(
  result: Pick<SingleResult, 'model' | 'thinkingLevel'>,
...
class RunProgressTracker {
  ...
  constructor(
    ...
    planned: Array<{ key: string; agent: string; task: string; step?: number }>,
  ) {
    this.mode = mode
  ...
  renderSummary(): string {
    ...
    if (this.mode === 'chain') {
      ...
      if (failed.length > 0)
        parts.push(
          `Failed: ${failed[0].label}${failed.length > 1 ? ` (+${failed.length - 1})` : ''}`,
        )
      return parts.join('\n')
    }
...
function attachProgressSummary(
  ...
  if (firstTextIndex >= 0) {
    ...
    if (current.type === 'text')
      content[firstTextIndex] = { ...current, text: `${summary}\n\n${current.text}` }
  } else {
...
function attachArtifactSummary(
  ...
  if (firstTextIndex >= 0) {
    const current = content[firstTextIndex]
    if (current.type === 'text')
      content[firstTextIndex] = { ...current, text: `${notice}\n\n${current.text}` }
  } else {
...
function getResultDisplayText(result: SingleResult): string {
  return (
    getFinalOutput(result.messages) || (result.exitCode === 0 ? '' : getResultErrorText(result))
  )
}
...
const GenericAgentSchema = Type.Object(
  {
    ...
    name: Type.Optional(
      Type.String({ description: 'Optional display name for an inline generic agent' }),
    ),
    description: Type.Optional(
      Type.String({ description: 'Optional description for the inline generic agent' }),
    ),
    tools: Type.Optional(
      Type.Array(Type.String(), {
        description: 'Optional tool allowlist for the inline generic agent',
      }),
    ),
    model: Type.Optional(
      Type.String({ description: 'Optional model override for the inline generic agent' }),
    ),
...
const StepItem = Type.Object(
  {
    ...
    task: Type.String({
      description:
        'Task to delegate to the agent. In sequential mode, {previous} can be used to inject the previous step output.',
    }),
    thinking: Type.Optional(ThinkingLevelSchema),
...
const SubagentParams = Type.Object(
  {
    steps: Type.Array(StepItem, {
      minItems: 1,
      description:
        'One step runs a single subagent; multiple steps run in parallel unless sequential:true.',
    }),
    sequential: Type.Optional(Type.Boolean({ description: 'Run steps sequentially as a chain.' })),
    cwd: Type.Optional(
      Type.String({
        description: 'Default working directory for all steps unless a step overrides it.',
      }),
    ),
    runTitle: Type.Optional(
      Type.String({ description: 'Optional tracking title for the parent subagent run.' }),
    ),
  },
  { additionalProperties: false },
)
...
const SubagentResultParams = Type.Object(
  {
    ...
    stepIndex: Type.Optional(
      Type.Number({
        minimum: 0,
        description: 'Optional zero-based step index to fetch a single step result.',
      }),
    ),
  },
  { additionalProperties: false },
...
function resolveAgentInvocation(
  defaultCwd: string,
  _requestedAgent: RequestedAgent,
...
function getSubagentMode(params: {
  steps: StepInput[]
  sequential?: boolean
...
function validateSubagentParams(params: {
  ...
  if (params.sequential && params.steps.length < 2)
    return 'Sequential mode requires at least 2 steps.'
  return undefined
}
...
function resolveTrackingCwd(
  params: { steps: StepInput[]; cwd?: string },
  sessionCwd: string,
): string {
  if (params.cwd) return resolveInvocationCwd(sessionCwd, params.cwd)
  const resolvedStepCwds = [
    ...new Set(params.steps.map(step => resolveStepCwd(step, params, sessionCwd))),
  ]
...
export default function (pi: ExtensionAPI) {
  ...
  const rememberModelContext = (ctx: {
  ...
  const findAgentForPreview = (agentName: string) =>
    discoverAgents(process.cwd(), 'user').agents.find(agent => agent.name === agentName)
  ...
  const formatPreviewSettings = (
    ...
    const providerText = resolvedParts.provider
      ? currentModel?.provider === resolvedParts.provider
        ? 'inherit'
        : resolvedParts.provider
      : 'inherit'
    const modelText = resolvedParts.id
      ? currentModel?.id === resolvedParts.id
        ? 'inherit'
        : resolvedParts.id
      : 'inherit'
    const requestedThinking = thinkingOverride ?? resolved.thinking
    const thinkingText =
      !requestedThinking || requestedThinking === 'inherit' ? 'inherit' : requestedThinking
    return ` ${theme.fg('dim', `[provider:${providerText}, model:${modelText}, thinking:${thinkingText}]`)}`
  }

  pi.on('session_start', async (_event, ctx) => {
    rememberModelContext(
      ctx as { model?: { provider: string; id: string }; modelRegistry: ModelRegistryLookup },
    )
  })

  pi.on('model_select', async (event, ctx) => {
    rememberModelContext({
      model: event.model as { provider: string; id: string },
      modelRegistry: ctx.modelRegistry as ModelRegistryLookup,
    })
  })
  ...
  pi.registerTool({
    ...
    async execute(toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (validationError) {
        const available =
          baseDiscovery.agents.map(a => `${a.name} (${a.source})`).join(', ') || 'none'
        return {
          content: [
            {
              type: 'text',
              text: `Invalid parameters. ${validationError}\nAvailable agents: ${available}`,
            },
          ],
      ...
      const emitProgressUpdate = (partial: AgentToolResult<SubagentDetails>) =>
        onUpdate?.(attachProgressSummary(partial, progressTracker))
      emitProgressUpdate({
        ...
        if (mode === 'chain') {
          ...
          for (let i = 0; i < params.steps.length; i++) {
            ...
            const invocation = resolveAgentInvocation(
              ctx.cwd,
              step.agent,
              resolveStepCwd(step, params, ctx.cwd),
            )
            ...
            try {
              ...
              emitProgressUpdate({
                content: [
                  { type: 'text', text: getFinalOutput(result.messages) || '(step complete)' },
                ],
                details: makeDetails('chain')(results),
              })
              ...
              if (isError) {
                ...
                return await finalizeAndReturn({
                  content: [
                    {
                      type: 'text',
                      text: `Chain stopped at step ${i + 1} (${getAgentDisplayName(step.agent)}): ${errorMsg}`,
                    },
                  ],
                  details: makeDetails('chain')(results),
                  isError: true,
          ...
          if (params.steps.length > MAX_PARALLEL_TASKS) {
            return await finalizeAndReturn({
              content: [
                {
                  type: 'text',
                  text: `Too many parallel tasks (${params.steps.length}). Max is ${MAX_PARALLEL_TASKS}.`,
                },
              ],
          ...
          for (let i = 0; i < params.steps.length; i++) {
            allResults[i] = {
              ...
              thinkingLevel: resolveThinkingLevel(
                ...
                isGenericAgentSpec(params.steps[i].agent)
                  ? params.steps[i].agent.thinking
                  : undefined,
                callerThinking,
              ),
            }
          ...
          const results = await mapWithConcurrencyLimit(
            ...
            async (t, index) => {
              ...
              const invocation = resolveAgentInvocation(
                ctx.cwd,
                t.agent,
                resolveStepCwd(t, params, ctx.cwd),
              )
              await todoTracker?.startTask(
                taskKey,
                agentName,
    ...
    renderResult(result, { expanded }, theme) {
      ...
      const artifactSummaryLines = details?.artifacts
        ? [`Artifacts: ${details.artifacts.runId}`, `Summary: ${details.artifacts.summaryFile}`]
      ...
      if (!details || details.results.length === 0) {
        ...
        if (details?.blocked) {
          ...
          return new Text(
            `${theme.fg('error', '✗')} ${theme.fg('error', message)}${theme.fg('muted', reason)}`,
            0,
            0,
          )
        }
      ...
      const appendSummaryHeader = (container: Container) => {
        if (
          summaryLines.progress.length === 0 &&
          artifactSummaryLines.length === 0 &&
          !summaryLines.todo
        )
          return
        for (const line of summaryLines.progress)
          container.addChild(new Text(theme.fg('accent', line), 0, 0))
        for (const line of artifactSummaryLines)
          container.addChild(new Text(theme.fg('muted', line), 0, 0))
        if (summaryLines.todo)
          container.addChild(new Text(theme.fg('muted', summaryLines.todo), 0, 0))
      ...
      if (details.mode === 'single' && details.results.length === 1) {
        ...
        if (expanded) {
          ...
          if (displayItems.length === 0 && !finalOutput) {
            container.addChild(
              new Text(
                isError ? theme.fg('error', errorText) : theme.fg('muted', '(no output)'),
                0,
                0,
              ),
            )
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      const normalizedStepIndex = Number.isFinite(params.stepIndex)
        ? Math.floor(params.stepIndex!)
        : undefined
      const queueResolveCwd = ctx.cwd

      try {
        if (normalizedStepIndex !== undefined) {
          const { run, step, content } = await readSubagentRunStep(
            queueResolveCwd,
            normalizedRunId,
            normalizedStepIndex,
          )
