...
import { mergeMetadata, Queue, resolveQueuePath } from '../../../sq-node/dist/index.js'
...
function parseStepArtifacts(value: unknown, runId: string): PersistedStepArtifact[] {
  ...
  return value.map((entry, index) => {
    if (!isRecord(entry))
      throw new Error(`sq run ${runId} has invalid step artifact at index ${index}`)
    ...
    return {
      ...
      thinkingLevel: getOptionalString(entry, 'thinkingLevel') as
...
function parsePersistedSubagentRunMetadata(
  metadata: Record<string, unknown>,
  runId: string,
): PersistedSubagentRunMetadata {
...
function buildStepMarkdown(
  runId: string,
  result: PersistableSubagentResult,
  artifact: PersistedStepArtifact,
): string {
  const lines: string[] = []
...
function buildSummaryMarkdown(run: PersistedSubagentRunMetadata): string {
  ...
  lines.push(``)
  lines.push(
    `Use \`subagent_result\` with this \`runId\` for the overview, or add \`stepIndex\` to fetch a single step.`,
  )
  lines.push(``)
  lines.push(`## Steps`)
  lines.push(``)
...
export async function readSubagentRunStep(
  ...
  if (!step) {
    ...
    throw new Error(
      `Run ${run.runId} has no stepIndex ${stepIndex}. Available step indexes: ${available}`,
    )
  }
  if (!fs.existsSync(step.outputFile)) {
    throw new Error(
      `Output file not found for run ${run.runId} stepIndex ${stepIndex}: ${step.outputFile}`,
    )
  }
  return {
    run,
