...
const SystemEvalParams = Type.Object(
  {
    candidatePrompt: Type.Optional(
      Type.String({
        description: 'Candidate AGENTS prompt file. Defaults to ~/.pi/agent/AGENTS.md',
      }),
    ),
    taskFile: Type.Optional(
      Type.String({ description: 'Task markdown file. Defaults to tools/prompt-eval/task.md' }),
    ),
    targetDiffFile: Type.Optional(
      Type.String({
        description: 'Frozen target diff file. Defaults to tools/prompt-eval/target.diff',
      }),
    ),
    baseRef: Type.Optional(
      Type.String({ description: 'Git ref used for the clean worktree. Defaults to origin/next' }),
    ),
    model: Type.Optional(
      Type.String({
        description:
          'Model for both the coding run and judge run. Defaults to github-copilot/gpt-5.4.',
      }),
    ),
    thinking: Type.Optional(
      Type.String({
        description: 'Thinking level for both the coding run and judge run. Defaults to medium.',
      }),
    ),
    timeoutSeconds: Type.Optional(
      Type.Number({
        description: 'Optional timeout for the coding run in seconds. Omit for no timeout.',
      }),
    ),
    keepWorktree: Type.Optional(
      Type.Boolean({ description: 'Keep the generated worktree after the run. Defaults to true' }),
    ),
  },
...
function parseJudgeResult(raw: string): JudgeResult {
  ...
  return {
    ...
    matchedIntent: Array.isArray(parsed.matchedIntent)
      ? parsed.matchedIntent.filter((item): item is string => typeof item === 'string')
      : [],
...
function formatProgressText(details: SystemEvalDetails, assistantPreview: string) {
  ...
  if (details.exitCode !== undefined)
    lines.push(`exit: ${details.exitCode}${details.timedOut ? ' (timed out)' : ''}`)
...
export default function registerSystemEvalExtension(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    promptSnippet:
      'Run a prompt evaluation in an isolated worktree and judge the produced diff against a saved target diff',
    promptGuidelines: [
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      if (process.env[AMP_SYSTEM_EVAL_PROCESS_ENV] === '1') {
        throw new Error(
          'system_eval cannot run recursively inside another system_eval child process.',
        )
      }
      ...
      const candidatePrompt = resolvePath(
        os.homedir(),
        params.candidatePrompt ?? path.join('~', '.pi', 'agent', 'AGENTS.md'),
      )
      const taskFile = params.taskFile
        ? resolvePath(ctx.cwd, params.taskFile)
        : path.join(repoRoot, DEFAULT_TASK_FILE)
      const targetDiffFile = params.targetDiffFile
        ? resolvePath(ctx.cwd, params.targetDiffFile)
        : path.join(repoRoot, DEFAULT_TARGET_DIFF_FILE)
      const baseRef = params.baseRef?.trim() || DEFAULT_BASE_REF
      const model = params.model?.trim() || DEFAULT_MODEL
      const thinking = params.thinking?.trim() || DEFAULT_THINKING
      const timeoutSeconds =
        Number.isFinite(params.timeoutSeconds) && (params.timeoutSeconds ?? 0) > 0
      ...
      const emitUpdate = (assistantText = '') => {
        onUpdate?.({
          content: [
            {
              type: 'text',
              text: formatProgressText(details, makeAssistantPreview(assistantText)),
            },
          ],
          details,
        })
      ...
      if (!fs.existsSync(candidatePrompt))
        throw new Error(`Candidate prompt file not found: ${candidatePrompt}`)
      if (!fs.existsSync(taskFile)) throw new Error(`Task file not found: ${taskFile}`)
      if (!fs.existsSync(targetDiffFile))
        throw new Error(`Target diff file not found: ${targetDiffFile}`)
      ...
      const addWorktreeResult = await runCommand(
        'git',
      ...
      if (addWorktreeResult.code !== 0) {
        throw new Error(
          addWorktreeResult.stderr || addWorktreeResult.stdout || 'Failed to create worktree.',
        )
      }
      ...
      details.changedFiles = statusResult.stdout
        .split('\n')
        .filter(line => line.trim().length > 0).length

      const diffResult = await runCommand('git', ['diff', '--no-ext-diff', '--binary'], {
        cwd: worktreeDir,
        signal,
      })
      ...
      if (judgeResult.exitCode !== 0) {
        throw new Error(
          judgeResult.stderr || `Judge run failed with exit code ${judgeResult.exitCode}.`,
        )
      }
        ...
        details.judge = {
          ...
          summary:
            error instanceof Error
              ? `Failed to parse judge output: ${error.message}`
