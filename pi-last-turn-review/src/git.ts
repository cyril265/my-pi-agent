...
function mergeChangedPaths(primary: ChangedPath[], secondary: ChangedPath[]): ChangedPath[] {
  const seen = new Set(primary.map((change) => `${change.status}:${change.oldPath ?? ""}:${change.newPath ?? ""}`));
...
export async function getLastTurnReviewFiles(pi: ExtensionAPI, repoRoot: string, beforeTree: string, afterTree: string): Promise<ReviewFile[]> {
  if (beforeTree === afterTree) return [];

  const output = await runGit(pi, repoRoot, ["diff", "--find-renames", "-M", "--name-status", beforeTree, afterTree, "--"]);
  return toReviewFiles(parseNameStatus(output));
}

export async function getGitChangesReviewFiles(pi: ExtensionAPI, repoRoot: string): Promise<ReviewFile[]> {
  if (await hasHead(pi, repoRoot)) {
    const trackedOutput = await runGit(pi, repoRoot, ["diff", "--find-renames", "-M", "--name-status", "HEAD", "--"]);
    const untrackedOutput = await runGitAllowFailure(pi, repoRoot, ["ls-files", "--others", "--exclude-standard"]);
    return toReviewFiles(mergeChangedPaths(parseNameStatus(trackedOutput), parseUntrackedPaths(untrackedOutput)));
  }

  const addedOutput = await runGitAllowFailure(pi, repoRoot, ["ls-files", "--cached", "--others", "--exclude-standard"]);
...
export async function loadLastTurnFileContents(
  ...
  const originalContent = comparison.oldPath == null ? "" : await getRevisionContent(pi, repoRoot, beforeTree, comparison.oldPath);
  const modifiedContent = comparison.newPath == null ? "" : await getRevisionContent(pi, repoRoot, afterTree, comparison.newPath);

  return { originalContent, modifiedContent };
}

export async function loadGitChangesFileContents(pi: ExtensionAPI, repoRoot: string, file: ReviewFile): Promise<ReviewFileContents> {
  const comparison = file.comparison;
