...
function normalizeCodexUsageWindow(window: unknown): CodexUsageWindow | undefined {
  ...
  return {
    ...
    windowSeconds:
      typeof raw.limit_window_seconds === 'number' ? raw.limit_window_seconds : undefined,
    resetAt: typeof raw.reset_at === 'number' ? raw.reset_at : undefined,
  }
}

function matchesCodexUsageWindow(
...
async function refreshCodexUsage(
  ...
  if (
    !options?.force &&
    cached?.fetchedAt &&
    Date.now() - cached.fetchedAt < CODEX_USAGE_CACHE_MS
  ) {
  ...
  try {
    ...
    const credential = ctx.modelRegistry.authStorage.get(providerName) as
      | { accountId?: unknown }
      | undefined
